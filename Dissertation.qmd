...
Xr_acf_graph <- Xr_autocorrelation |>
  ...
  ggtitle("Superimposed ACFs of All Stocks") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
...
Xr_pacf_graph <- Xr_autocorrelation |>
  ...
  geom_hline(aes(yintercept = -ci), linetype = "dashed") +
  geom_hline(yintercept = 0) +
  ggtitle("Superimposed PACFs of All Stocks") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
```
...
dcsis_glmnet_function <- function(regression, grid) {
  ...
  dcsis_glmnet <- folds |>
    map(function(fold) {
      ...
      if (regression == "adlasso") {
        ridge <- glmnet_function(YXr_reduced, regression, ridge_grid)
        ridge_coefs <- ridge$model_coefs$method
        penalty_factor <- 1 / (abs(ridge_coefs) + 1e-5)
      } else {
...
en1_dcsis_plot <- plot_model(
  ...
  "Elastic Net Regression (alpha = 0.5, DC-SIS)",
  0.2,
  0.9
)
```
...
ggsave("my_plot.png", plot = graph_plot_models$`3`, width = 8, height = 5, dpi = 300)
```
#

### 2.6.2 Regularization Parameters Used
...
### 2.6.4 Number of Variables with Non-Zero Coefficients
...
model_variables <- list_model_variables |>
  bind_cols()
```
...
### 3.1.1 Compute the weight of ETF returns

```{r}
ETF_str <- list_model_coefficients |>
  map(function(model) {
    coef_col <- model[[2]]
    model |>
      mutate(
...
ETF <- model_names |>
  ...
  map(function(model) {
    ...
    stocks <- ETF_str[[model]]$stock
    weights <- ETF_str[[model]]$weight_returns
...
### 3.1.3 Compute ETF's stock weighting

```{r}
ETF_weighting <- model_names |>
  set_names() |>
  map(function(model) {
    # Get stock symbols
    stocks <- ETF_str[[model]]$stock

    # Compute ETF weighting
    X |>
      ...
      group_by(date) |>
      mutate(weight = stock_price / sum(stock_price)) |>
      select(date, stock, weight) |>
      ungroup() |>
